...
from backend.LiveExercise.AiTrainer_utils import *
import joblib
...
def count_repetition_push_up(detector, img, landmark_list, stage, counter, exercise_instance):
    ...
    return stage, counter, increment
...
def count_repetition_bicep_curl(detector, img, landmark_list, stage_right, stage_left, counter, exercise_instance):
    ...
    return stage_right, stage_left, counter, increment
...
class Exercise:
    def __init__(self):
        try:
            self.lstm_model = load_model('models/bilstm_model.h5')
    ...
    #         print("In condition")
    #         for i in range(len(relevant_landmarks_indices)):
    #             start = i * 3
    #             features.extend(landmarks[start:start + 3])
    ...
    def extract_features(self, landmarks):
        ...
        if len(landmarks) == len(relevant_landmarks_indices) * 3:
            ...
            # 12 angles (same as in training)
            angles = [
                calculate_angle(landmarks[18:21], landmarks[0:3], landmarks[6:9]),    # LEFT_HIP, LEFT_SHOULDER, LEFT_ELBOW
                calculate_angle(landmarks[21:24], landmarks[3:6], landmarks[9:12]),   # RIGHT_HIP, RIGHT_SHOULDER, RIGHT_ELBOW
                calculate_angle(landmarks[0:3], landmarks[6:9], landmarks[12:15]),    # LEFT_SHOULDER, LEFT_ELBOW, LEFT_WRIST
                calculate_angle(landmarks[3:6], landmarks[9:12], landmarks[15:18]),   # RIGHT_SHOULDER, RIGHT_ELBOW, RIGHT_WRIST
                calculate_angle(landmarks[18:21], landmarks[24:27], landmarks[30:33]),# LEFT_HIP, LEFT_KNEE, LEFT_ANKLE
                calculate_angle(landmarks[21:24], landmarks[27:30], landmarks[33:36]),# RIGHT_HIP, RIGHT_KNEE, RIGHT_ANKLE
                calculate_angle(landmarks[0:3], landmarks[18:21], landmarks[24:27]),  # LEFT_SHOULDER, LEFT_HIP, LEFT_KNEE
                calculate_angle(landmarks[3:6], landmarks[21:24], landmarks[27:30]),  # RIGHT_SHOULDER, RIGHT_HIP, RIGHT_KNEE
                calculate_angle(landmarks[24:27], landmarks[30:33], landmarks[36:39]),# LEFT_KNEE, LEFT_ANKLE, LEFT_HEEL
                calculate_angle(landmarks[27:30], landmarks[33:36], landmarks[39:42]),# RIGHT_KNEE, RIGHT_ANKLE, RIGHT_HEEL
                calculate_angle(landmarks[30:33], landmarks[36:39], landmarks[42:45]),# LEFT_ANKLE, LEFT_HEEL, LEFT_FOOT_INDEX
                calculate_angle(landmarks[33:36], landmarks[39:42], landmarks[45:48]) # RIGHT_ANKLE, RIGHT_HEEL, RIGHT_FOOT_INDEX
            ]
            features.extend(angles)

            # # Optional: normalize coordinates using shoulder-hip or hip-knee distance
            # normalization_factor = -1
            # distances_to_check = [
            #     calculate_distance(landmarks[0:3], landmarks[18:21]),  # LEFT_SHOULDER, LEFT_HIP
            #     calculate_distance(landmarks[3:6], landmarks[21:24]),  # RIGHT_SHOULDER, RIGHT_HIP
            ...
            #     if d > 0:
            #         normalization_factor = d
            #         break
            # if normalization_factor == -1:
            #     normalization_factor = 0.5  # fallback

            # # Normalize only the 66 coordinate features (not the angles)
            # for i in range(0, 66, 3):
            #     features[i]     /= normalization_factor  # x
            #     features[i + 1] /= normalization_factor  # y
            #     features[i + 2] /= normalization_factor  # z

        else:
            print(f"Insufficient landmarks: expected {len(relevant_landmarks_indices)}, got {len(landmarks)//3}")
            features = [-1.0] * 78  # fallback
    ...
    #     try:
    #         if len(landmarks) == len(relevant_landmarks_indices) * 3:
    #             # 1. Extract 3D coordinates (66 features)
    #             for i in range(len(relevant_landmarks_indices)):
    #                 start = i * 3
    #                 features.extend(landmarks[start:start + 3])

    #             # 2. Calculate angles (6 angles)
    #             angles = [
    #                 calculate_angle(landmarks[0:3], landmarks[6:9], landmarks[12:15]),   # LEFT_SHOULDER, LEFT_ELBOW, LEFT_WRIST
    #                 calculate_angle(landmarks[3:6], landmarks[9:12], landmarks[15:18]),  # RIGHT_SHOULDER, RIGHT_ELBOW, RIGHT_WRIST
    #                 calculate_angle(landmarks[18:21], landmarks[24:27], landmarks[30:33]),  # LEFT_HIP, LEFT_KNEE, LEFT_ANKLE
    ...
    #             # 3. Distances (6 distances)
    #             distances = [
    #                 calculate_distance(landmarks[0:3], landmarks[3:6]),     # Shoulder width
    #                 calculate_distance(landmarks[18:21], landmarks[21:24]), # Hip width
    #                 calculate_distance(landmarks[6:9], landmarks[24:27]),   # LEFT_ELBOW, LEFT_KNEE
    #                 calculate_distance(landmarks[9:12], landmarks[27:30]),  # RIGHT_ELBOW, RIGHT_KNEE
    #                 calculate_distance(landmarks[12:15], landmarks[18:21]), # LEFT_WRIST, LEFT_HIP
    #                 calculate_distance(landmarks[15:18], landmarks[21:24])  # RIGHT_WRIST, RIGHT_HIP
    ...
    #             ]
    #             normalization_factor = next((d for d in norm_candidates if d > 0.01), 0.5)

    #             # 5. Normalize distances
    #             normalized_distances = [d / normalization_factor if d != -1.0 else -1.0 for d in distances]
    ...
    #         features = [-1.0] * 78  # fallback on error
